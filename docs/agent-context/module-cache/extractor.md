...
- Source anchors: `DealData` schema validators; `EXTRACTION_PROMPT`; `SECTORS`; `SUB_SECTORS`; Groq call settings in `DealExtractor`.
...
- Verification: `python -m pytest tests/test_extractor_schema.py`; live Groq checks only when prompt quality is the actual change.
