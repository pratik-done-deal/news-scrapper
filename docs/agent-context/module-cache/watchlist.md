...
- `GET /api/v1/news/companies/watchlist` — preview: totals, counts by type, derived terms. Cheap; call before spending a run.
- `POST /api/v1/news/companies/scrape/watchlist` — 202 + `ScrapeJobResponse`, polled via the existing `GET /companies/scrape/{job_id}`.
