...
def test_the_endpoint_being_called_is_sent_not_the_validate_endpoint():
    ...
    client.validate("sess-1", "/api/v1/deals/{deal_id}")

    assert http.post.call_args.kwargs["json"] == {"apiEndPoint": "/api/v1/deals/{deal_id}"}
...
def test_the_session_id_travels_as_a_bearer_token():
    ...
    client.validate("sess-1", "/api/v1/deals")
...
def test_a_valid_session_is_parsed_into_its_fields():
    ...
    session = client.validate("sess-1", "/api/v1/deals")
...
def test_the_same_session_on_a_different_endpoint_is_validated_again():
    ...
    client.validate("sess-1", "/api/v1/deals")
    client.validate("sess-1", "/api/v1/articles")
...
def build_app(auth_client=None):
    ...
    @app.post("/api/v1/tracked-companies")
    def register():
...
def test_a_request_with_no_token_is_401_without_calling_the_auth_service():
    ...
    response = TestClient(build_app(client)).get("/api/v1/deals/d1")
...
def test_a_valid_session_reaches_the_route():
    response = TestClient(build_app()).get(
        "/api/v1/deals/d1", headers={"Authorization": "Bearer sess-1"}
    )
...
def test_the_done_deal_push_endpoint_answers_without_a_token():
    ...
    response = TestClient(build_app(client)).post("/api/v1/tracked-companies")
...
def test_auth_disabled_lets_everything_through(monkeypatch):
    ...
    response = TestClient(build_app(client)).get("/api/v1/articles")
...
def test_auth_enabled_guards_a_route_that_never_asks_who_is_calling():
    ...
    assert test_client.get("/api/v1/articles").status_code == 401
    assert test_client.get(
        "/api/v1/articles", headers={"Authorization": "Bearer sess-1"}
    ).status_code == 200
    assert http.post.call_count == 1
