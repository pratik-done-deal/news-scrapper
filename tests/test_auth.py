...
def test_the_endpoint_being_called_is_sent_not_the_validate_endpoint():
    ...
    client.validate("sess-1", "/api/news-scrapper/deals/{deal_id}")
...
def test_a_valid_session_is_parsed_into_its_fields():
    ...
    session = client.validate("sess-1", "/api/news-scrapper/deals")
...
def test_the_same_session_on_a_different_endpoint_is_validated_again():
    ...
    client.validate("sess-1", "/api/news-scrapper/deals")
    client.validate("sess-1", "/api/news-scrapper/articles")
...
def build_app(auth_client=None):
    ...
    @app.post("/api/news-scrapper/tracked-companies")
    def register():
        return {"registered": True}

    @app.get("/api/news-scrapper/deals/{deal_id}")
    ...
    @app.get("/api/news-scrapper/articles")
    def articles():
...
def test_a_request_with_no_token_is_401_without_calling_the_auth_service():
    ...
    response = TestClient(build_app(client)).get("/api/news-scrapper/deals/d1")
...
def test_the_done_deal_push_endpoint_answers_without_a_token():
    ...
    response = TestClient(build_app(client)).post("/api/news-scrapper/tracked-companies")
...
def test_auth_disabled_lets_everything_through():
    ...
    response = TestClient(build_app(client)).get("/api/news-scrapper/articles")
