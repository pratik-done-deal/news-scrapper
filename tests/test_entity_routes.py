...
def build_client(rows):
    app = FastAPI()
    app.include_router(entities.router, prefix="/api/news")
...
def test_news_feed_is_looked_up_by_the_resolved_name():
    ...
    with patch(
        ...
        response = client.get("/api/news/entities/S5123/news")
...
def test_untracked_entity_is_a_404():
    ...
    response = client.get("/api/news/entities/S9999/news")
...
def test_malformed_reference_is_a_400(ref):
    ...
    response = client.get(f"/api/news/entities/{ref}/news")
...
def test_entity_lookup_without_fetching_news():
    ...
    response = client.get("/api/news/entities/S5123")
