...
Then point the API at it and start the API in another shell:
    ...
    curl localhost:8000/api/v1/deals                              # 401
    curl -H 'Authorization: Bearer good-session' \
         localhost:8000/api/v1/deals                              # 200
    curl -H 'Authorization: Bearer good-session' \
         localhost:8000/api/v1/analytics/deals/volume             # 403
...
FORBIDDEN_ENDPOINTS = {"/api/v1/analytics/deals/volume"}
